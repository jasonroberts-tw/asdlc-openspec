 ...
 *
 * THE FAILURE THIS PREVENTS. `package.json` once spelled a Python gate's scripts `python3 ...`, which is
 ...
 * so that pre-push job went red on one machine with no output from the gate itself.
 ...
 * the Store alias first, and on one Windows machine that alias takes 1.9-2.2s to fail with exit 9009
 * (measured 2026-09-09) -- a probe the header used to call "milliseconds" was the slowest step of
 * the gate. So win32 tries the launcher `py -3` and then `python` (0.65s each there) before
