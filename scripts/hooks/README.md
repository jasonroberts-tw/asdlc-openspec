...
| `worktree-hooks.selftest.mjs` | none: `worktree:selftest` | | The negative tests for the two worktree hooks, the guard and the branch sweep, and the check that every command `.claude/settings.json` registers loads its script from any working directory. Each case asserts the reason a refusal reports, not the refusal alone. A pre-push job. |
