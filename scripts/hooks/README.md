...
| `guard-git.mjs` | `PreToolUse` on Bash | yes; its git rules in a linked worktree only | Tokenises the command and refuses, from a worktree, a push, checkout, branch write or merge against a protected branch, and `git worktree`, `gc` and `prune`. From any checkout, it refuses a `gh pr create` that does not name `main` as its base, and a `gh` command that applies the reviewer's approval label, whose spelling it reads from `tools/policy.json` (`docs/decisions.md` § R-01). Where the command runs is the payload's `cwd`, never `CLAUDE_PROJECT_DIR`, which stays at the primary checkout after `EnterWorktree`. Fails closed in a worktree when it cannot read its input. |
