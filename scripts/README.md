...
| `workflows.selftest.mjs` | selftest, `workflows:selftest` | Runs `.claude/workflows/build-change-task.js` against stubbed agents with the review sizes in `tools/policy.json`, and refuses a workflow that stops, confirms, merges or reports other than its header says, or that the Workflow runtime would not accept: a `meta` that is not a pure literal first, a phase `meta` does not declare, or a clock or random read. `WORKFLOWS_ROOT` points it at a doctored copy. |
