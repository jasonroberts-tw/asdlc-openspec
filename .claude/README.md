...
| `workflows/<name>.js` | Workflow scripts a skill runs through the Workflow tool, each named by the skill that runs it. `build-change-task.js` builds and reviews one task of a product change (`change-build` step 3), with the sizes `tools/policy.json` gives it. `check:prompts` does not read these: each opens with the `meta` literal the runtime needs first, with its header right after, and `workflows:selftest` runs each against stubbed agents. |
