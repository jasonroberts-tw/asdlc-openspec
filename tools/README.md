...
| `policy.json` | policy file | The workflow's constants that belong to no one tool, each under a key with a `<key>Means` sibling (`docs/decisions.md` § D-03). Today one: `specChangeLabel`, the label on a product change's epic and tasks. `openspec:check` refuses a skill or agent that spells the label without citing the key, or cites it and spells another value. The learning loop's own constants are in `outcomes/policy.json`. |
