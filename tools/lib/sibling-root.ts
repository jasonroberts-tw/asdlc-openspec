/**
 * sibling-root.ts — where the sibling checkout this repository measures lives, and a git
 * environment that is safe to ask it questions with.
 *
 * EMITS. Nothing. Three functions: `gitEnv()`, `primaryCheckout(from)`, `resolveSiblingRoot(from)`.
 *
 ...
 * and `GIT_INDEX_FILE` to every hook, and `GIT_DIR` outranks `-C <dir>`: a `git -C ../sibling
 * rev-parse HEAD` run from inside a pre-push hook answers for THIS repository, and a provenance
 * stamp records this repository's commit as the sibling's. (2) A linked worktree lives under
 ...
 * (`existsSync(resolveSiblingRoot(root))`), and a legitimately absent checkout skips clean and says
 * why. Set `SIBLING_ROOT` to point a by-hand run, or a selftest, at a fixture.
 *
 * kit 2.6-4 · ADAPT: `../sibling` is a stand-in for whatever sibling checkout your emitters read.
 * Rename the directory in `resolveSiblingRoot` to yours; delete this file, and the `checkout` input
...
const SIBLING_DIRECTORY = 'sibling'
...
/** The sibling checkout's root: `SIBLING_ROOT` when set, else `../sibling` beside the primary checkout. */
export function resolveSiblingRoot(from: string): string {
  const override = process.env['SIBLING_ROOT']
  if (override) return resolve(override)
  return resolve(primaryCheckout(from), '..', SIBLING_DIRECTORY)
}
